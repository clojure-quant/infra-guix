...
; http://localhost:8384/
; Two devices will only connect and talk to each other if they are both configured with each
; other’s device ID. 
; A device ID is a unique, cryptographically-secure identifier that is generated as part of 
; the key generation the first time you start Syncthing. 
; It is printed in the log above, and you can see it in the web GUI by 
; selecting “Actions” (top right) and “Show ID
